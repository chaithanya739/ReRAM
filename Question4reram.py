...
for j in range(2):
    ...
    vector = np.zeros(num_of_nodes)
    for i in range(N):
        matrix,vector = voltage_source_stamp(matrix,vector,i+1,0,0.8,num_nodes)
        num_nodes += 1
    count = N
    matrix,vector = voltage_source_stamp(matrix,vector,count,0,2,num_nodes)
    count += 1
    for i in range(N-1):
        matrix,vector = mosfet_stamp(matrix,vector,N,i+1,count,Vth,K,Lambda,Vin[i]-Vs[i],Vdd-Vs[i])
        matrix,vector = resistor_stamp(matrix,vector,count,count+1,6)
        matrix,vector = resistor_stamp(matrix,vector,count+1,count+2,6)
        count += 3
        matrix,vector = mosfet_stamp(matrix,vector,N,i+1,count,Vth,K,Lambda,Vin[i]-Vs[i],Vdd-Vs[i])
        matrix,vector = resistor_stamp(matrix,vector,count,count+1,6)
        matrix,vector = resistor_stamp(matrix,vector,count+1,count+2,6)
        count += 3
    matrix,vector = mosfet_stamp(matrix,vector,N,N-1,count,Vth,K,Lambda,Vin[N-1]-Vs[N-1],Vdd-Vs[N-1])
    matrix,vector = resistor_stamp(matrix,vector,count,count+1,6)
    matrix,vector = resistor_stamp(matrix,vector,count+1,0,6)
    count += 2
    matrix,vector = mosfet_stamp(matrix,vector,N,N-1,count,Vth,K,Lambda,Vin[N-1]-Vs[N-1],Vdd-Vs[N-1])
    matrix,vector = resistor_stamp(matrix,vector,count,count+1,6)
    matrix,vector = resistor_stamp(matrix,vector,count+1,0,6)
